...
void trasformazione2(double* x, int n, double t) {
	double punto[2];
	punto[0]= (S1-(S1-S2*S2)*t)*cos(3*PI/4.-(3*PI/4.-133.0140178*PI/180.+2*PI)*t)*x[0]-(S1-(S1-S2*S2)*t)*sin(3*PI/4.-(3*PI/4.-133.0140178*PI/180.+2*PI)*t)*x[1]+1; 
	punto[1]= (S1-(S1-S2*S2)*t)*sin(3*PI/4.-(3*PI/4.-133.0140178*PI/180.+2*PI)*t)*x[0]+(S1-(S1-S2*S2)*t)*cos(3*PI/4.-(3*PI/4.-133.0140178*PI/180.+2*PI)*t)*x[1]; 
		glColor3ub(0,0,255);
		glVertex2f((punto[0]-0.6f)/SCALA,(punto[1]-0.4f)/SCALA);
	ricorsione(punto,n,t);  
}
